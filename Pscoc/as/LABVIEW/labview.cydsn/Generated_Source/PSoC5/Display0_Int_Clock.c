/*******************************************************************************
* File Name: Display0_Int_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Display0_Int_Clock_CLKEN |= Display0_Int_Clock_CLKEN_MASK;
	Display0_Int_Clock_CLKSTBY |= Display0_Int_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Display0_Int_Clock_CLKEN &= (uint8)(~Display0_Int_Clock_CLKEN_MASK);
	Display0_Int_Clock_CLKSTBY &= (uint8)(~Display0_Int_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_StopBlock(void) 
{
    if ((Display0_Int_Clock_CLKEN & Display0_Int_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(Display0_Int_Clock__CFG3)
        CLK_DIST_AMASK = Display0_Int_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Display0_Int_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Display0_Int_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Display0_Int_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Display0_Int_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Display0_Int_Clock_CLKSTBY &= (uint8)(~Display0_Int_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Display0_Int_Clock_GetSourceRegister();
    uint16 oldDivider = Display0_Int_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Display0_Int_Clock_CLKEN & Display0_Int_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Display0_Int_Clock_DIV_PTR, clkDivider);
                Display0_Int_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Display0_Int_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Display0_Int_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Display0_Int_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Display0_Int_Clock_DIV_PTR, clkDivider);
				Display0_Int_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Display0_Int_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Display0_Int_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_SetModeRegister(uint8 modeBitMask) 
{
    Display0_Int_Clock_MOD_SRC |= modeBitMask & (uint8)Display0_Int_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    Display0_Int_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Display0_Int_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display0_Int_Clock_GetModeRegister(void) 
{
    return Display0_Int_Clock_MOD_SRC & (uint8)(Display0_Int_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Display0_Int_Clock_GetDividerRegister();
    uint8 oldSrc = Display0_Int_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Display0_Int_Clock_MOD_SRC |= CYCLK_SSS;
        Display0_Int_Clock_MOD_SRC =
            (Display0_Int_Clock_MOD_SRC & (uint8)(~Display0_Int_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display0_Int_Clock_GetSourceRegister(void) 
{
    return Display0_Int_Clock_MOD_SRC & Display0_Int_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Display0_Int_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display0_Int_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    Display0_Int_Clock_PHASE = clkPhase & Display0_Int_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Display0_Int_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display0_Int_Clock_GetPhaseRegister(void) 
{
    return Display0_Int_Clock_PHASE & Display0_Int_Clock_PHASE_MASK;
}
