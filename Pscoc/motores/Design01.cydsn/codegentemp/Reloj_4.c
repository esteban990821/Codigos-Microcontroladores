...
void Reloj_4_Start(void) 
{
    ...
    Reloj_4_CLKEN |= Reloj_4_CLKEN_MASK;
	Reloj_4_CLKSTBY |= Reloj_4_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Reloj_4_Stop
********************************************************************************
*
* Summary:
...
void Reloj_4_Stop(void) 
{
    ...
    Reloj_4_CLKEN &= (uint8)(~Reloj_4_CLKEN_MASK);
	Reloj_4_CLKSTBY &= (uint8)(~Reloj_4_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Reloj_4_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Reloj_4_CLKEN & Reloj_4_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Reloj_4_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Reloj_4_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Reloj_4_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Reloj_4_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Reloj_4_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Reloj_4_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Reloj_4_GetSourceRegister();
    uint16 oldDivider = Reloj_4_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Reloj_4_CLKEN & Reloj_4_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Reloj_4_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Reloj_4_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Reloj_4_DIV_PTR);
}


/*******************************************************************************
* Function Name: Reloj_4_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_4_MOD_SRC |= modeBitMask & (uint8)Reloj_4_MODE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_4_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_4_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Reloj_4_MODE_MASK));
}


/*******************************************************************************
* Function Name: Reloj_4_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_4_MOD_SRC & (uint8)(Reloj_4_MODE_MASK);
}


/*******************************************************************************
* Function Name: Reloj_4_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Reloj_4_GetDividerRegister();
    uint8 oldSrc = Reloj_4_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Reloj_4_MOD_SRC |= CYCLK_SSS;
        Reloj_4_MOD_SRC =
            (Reloj_4_MOD_SRC & (uint8)(~Reloj_4_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Reloj_4_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_4_MOD_SRC & Reloj_4_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Reloj_4_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_4_PHASE = clkPhase & Reloj_4_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_4_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Reloj_4_PHASE & Reloj_4_PHASE_MASK;
}
