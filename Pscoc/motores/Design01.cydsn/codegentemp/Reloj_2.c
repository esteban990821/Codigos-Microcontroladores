...
void Reloj_2_Start(void) 
{
    ...
    Reloj_2_CLKEN |= Reloj_2_CLKEN_MASK;
	Reloj_2_CLKSTBY |= Reloj_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Reloj_2_Stop
********************************************************************************
*
* Summary:
...
void Reloj_2_Stop(void) 
{
    ...
    Reloj_2_CLKEN &= (uint8)(~Reloj_2_CLKEN_MASK);
	Reloj_2_CLKSTBY &= (uint8)(~Reloj_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Reloj_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Reloj_2_CLKEN & Reloj_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Reloj_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Reloj_2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Reloj_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Reloj_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Reloj_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Reloj_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Reloj_2_GetSourceRegister();
    uint16 oldDivider = Reloj_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Reloj_2_CLKEN & Reloj_2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Reloj_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Reloj_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Reloj_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Reloj_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_2_MOD_SRC |= modeBitMask & (uint8)Reloj_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Reloj_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: Reloj_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_2_MOD_SRC & (uint8)(Reloj_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: Reloj_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Reloj_2_GetDividerRegister();
    uint8 oldSrc = Reloj_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Reloj_2_MOD_SRC |= CYCLK_SSS;
        Reloj_2_MOD_SRC =
            (Reloj_2_MOD_SRC & (uint8)(~Reloj_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Reloj_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_2_MOD_SRC & Reloj_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Reloj_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_2_PHASE = clkPhase & Reloj_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Reloj_2_PHASE & Reloj_2_PHASE_MASK;
}
