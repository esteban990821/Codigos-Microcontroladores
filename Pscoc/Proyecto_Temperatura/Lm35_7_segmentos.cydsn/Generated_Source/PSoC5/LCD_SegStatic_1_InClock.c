/*******************************************************************************
* File Name: LCD_SegStatic_1_InClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_Start
********************************************************************************
*
* Summary:
...
void LCD_SegStatic_1_InClock_Start(void) 
{
    ...
    LCD_SegStatic_1_InClock_CLKEN |= LCD_SegStatic_1_InClock_CLKEN_MASK;
	LCD_SegStatic_1_InClock_CLKSTBY |= LCD_SegStatic_1_InClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_Stop
********************************************************************************
*
* Summary:
...
void LCD_SegStatic_1_InClock_Stop(void) 
{
    ...
    LCD_SegStatic_1_InClock_CLKEN &= (uint8)(~LCD_SegStatic_1_InClock_CLKEN_MASK);
	LCD_SegStatic_1_InClock_CLKSTBY &= (uint8)(~LCD_SegStatic_1_InClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LCD_SegStatic_1_InClock_CLKEN & LCD_SegStatic_1_InClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LCD_SegStatic_1_InClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LCD_SegStatic_1_InClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LCD_SegStatic_1_InClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LCD_SegStatic_1_InClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LCD_SegStatic_1_InClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LCD_SegStatic_1_InClock_GetSourceRegister();
    uint16 oldDivider = LCD_SegStatic_1_InClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LCD_SegStatic_1_InClock_CLKEN & LCD_SegStatic_1_InClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LCD_SegStatic_1_InClock_DIV_PTR, clkDivider);
                LCD_SegStatic_1_InClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LCD_SegStatic_1_InClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LCD_SegStatic_1_InClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LCD_SegStatic_1_InClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LCD_SegStatic_1_InClock_DIV_PTR, clkDivider);
				LCD_SegStatic_1_InClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LCD_SegStatic_1_InClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LCD_SegStatic_1_InClock_MOD_SRC |= modeBitMask & (uint8)LCD_SegStatic_1_InClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LCD_SegStatic_1_InClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LCD_SegStatic_1_InClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LCD_SegStatic_1_InClock_MOD_SRC & (uint8)(LCD_SegStatic_1_InClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LCD_SegStatic_1_InClock_GetDividerRegister();
    uint8 oldSrc = LCD_SegStatic_1_InClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LCD_SegStatic_1_InClock_MOD_SRC |= CYCLK_SSS;
        LCD_SegStatic_1_InClock_MOD_SRC =
            (LCD_SegStatic_1_InClock_MOD_SRC & (uint8)(~LCD_SegStatic_1_InClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LCD_SegStatic_1_InClock_MOD_SRC & LCD_SegStatic_1_InClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LCD_SegStatic_1_InClock_PHASE = clkPhase & LCD_SegStatic_1_InClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LCD_SegStatic_1_InClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LCD_SegStatic_1_InClock_PHASE & LCD_SegStatic_1_InClock_PHASE_MASK;
}
