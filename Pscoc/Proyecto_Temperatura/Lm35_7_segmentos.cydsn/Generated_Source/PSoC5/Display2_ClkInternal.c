/*******************************************************************************
* File Name: Display2_ClkInternal.c
* Version 2.20
*
*  Description:
...
#include "Display2_ClkInternal.h"
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_Start
********************************************************************************
*
* Summary:
...
void Display2_ClkInternal_Start(void) 
{
    ...
    Display2_ClkInternal_CLKEN |= Display2_ClkInternal_CLKEN_MASK;
	Display2_ClkInternal_CLKSTBY |= Display2_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
void Display2_ClkInternal_Stop(void) 
{
    ...
    Display2_ClkInternal_CLKEN &= (uint8)(~Display2_ClkInternal_CLKEN_MASK);
	Display2_ClkInternal_CLKSTBY &= (uint8)(~Display2_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Display2_ClkInternal_CLKEN & Display2_ClkInternal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Display2_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Display2_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Display2_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Display2_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Display2_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Display2_ClkInternal_GetSourceRegister();
    uint16 oldDivider = Display2_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Display2_ClkInternal_CLKEN & Display2_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Display2_ClkInternal_DIV_PTR, clkDivider);
                Display2_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Display2_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Display2_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Display2_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Display2_ClkInternal_DIV_PTR, clkDivider);
				Display2_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Display2_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Display2_ClkInternal_MOD_SRC |= modeBitMask & (uint8)Display2_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Display2_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Display2_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Display2_ClkInternal_MOD_SRC & (uint8)(Display2_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Display2_ClkInternal_GetDividerRegister();
    uint8 oldSrc = Display2_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Display2_ClkInternal_MOD_SRC |= CYCLK_SSS;
        Display2_ClkInternal_MOD_SRC =
            (Display2_ClkInternal_MOD_SRC & (uint8)(~Display2_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Display2_ClkInternal_MOD_SRC & Display2_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Display2_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Display2_ClkInternal_PHASE = clkPhase & Display2_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Display2_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Display2_ClkInternal_PHASE & Display2_ClkInternal_PHASE_MASK;
}
