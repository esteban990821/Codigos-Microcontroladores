...
#include <Display2_Frame_Dma_dma.h>


/* Internal functions */
static uint8   Display2_DmaConfigure(void) CYREENTRANT;
static void    Display2_DmaDispose(void) CYREENTRANT;
static void    Display2_WriteControlReg(uint8 value, uint8 mask) CYREENTRANT;
...
*/
  ...
  static const uint8 CYCODE Display2_7SegDigits[] = \
    /*    '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7' */
    ...
    #ifdef Display2_14SEG

        static const uint16 CYCODE Display2_14SegChars[] = {
        /*------------------------------------------------------------*/
        /*                           Blank                            */
        /*------------------------------------------------------------*/
    ...
    #endif /* Display2_14SEG */

    #ifdef Display2_16SEG

        static const uint16 CYCODE Display2_16SegChars[] = {
        /*------------------------------------------------------------*/
        /*                           Blank                            */
        /*------------------------------------------------------------*/
        0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,
    ...
    #endif /* Display2_16SEG */

    #ifdef Display2_DOT_MATRIX
        static const uint8 CYCODE Display2_charDotMatrix[][5u] = {
    ...
    #endif /* Display2_DOT_MATRIX */
#endif /* ALPHANUMERIC */

uint8 Display2_initVar = 0u;

/* Stores DMA  channel number used for Frame signal transaction */
static uint8 Display2_frameChannel;

/* The one and only - Frame Buffer */
static uint8 Display2_buffer[Display2_BUFFER_LENGTH];

/* Array of common port TDs */
static uint8 Display2_lcdTd[Display2_LCD_TD_SIZE * Display2_COMM_NUM];

/* DMA channel dedicated for SegLCD commons */
static uint8 Display2_lcdChannel;

/* Start of customizer generated code */

static uint16 Display2_commons[1u] = {Display2_Com0};

static uint8 Display2_frameTx[2u] = {0x80u, 0x00u};
static uint8 Display2_frameTd[2u];
...
void Display2_Init(void) 
{
    ...
    #if(Display2_POWER_MODE == Display2__LOW_POWER_ILO)

        /* Select ILO as a wake up source */
        Display2_TIMER_CONTROL_REG = 
            Display2_TIMER_CONTROL_REG & ((uint8) ~Display2_TIMER_CLK_SEL_MASK);
        /* Set period */
        Display2_TIMER_CONTROL_REG = (Display2_TIMER_CONTROL_REG & 
                            ((uint8) ~Display2_TIMER_PERIOD_MASK))
                            | (uint8) (((uint8) Display2_TIMER_PERIOD) << Display2_TIMER_PERIOD_SHIFT);

    #elif(Display2_POWER_MODE == Display2__LOW_POWER_32XTAL)
        /* One PPS must be enabled to generate 8Khz clock for LCD Timer */
        Display2_TM_WL_GFG_REG |= Display2_ONE_PPS_EN;
        /* Select 8K tap from OPPS timer as a wake up source */
        Display2_TIMER_CONTROL_REG = Display2_TIMER_CONTROL_REG | Display2_TIMER_32XTAL_SEL;
        /* Set period */
        Display2_TIMER_CONTROL_REG = (Display2_TIMER_CONTROL_REG &
                           ((uint8) ~Display2_TIMER_PERIOD_MASK))
                           | (uint8) (((uint8) Display2_TIMER_PERIOD) << Display2_TIMER_PERIOD_SHIFT);

    #endif /* Display2_POWER_MODE == Display2__LOW_POWER_ILO */

    /* Need to clear display to start normal work */
    Display2_ClearDisplay();

    /* Select LCD DAC generated voltage as the source for LCD Driver */
    Display2_LCDDAC_SWITCH_REG0_REG = Display2_LCDDAC_VOLTAGE_SEL;
    Display2_LCDDAC_SWITCH_REG1_REG = Display2_LCDDAC_VOLTAGE_SEL;
    Display2_LCDDAC_SWITCH_REG2_REG = Display2_LCDDAC_VOLTAGE_SEL;
    Display2_LCDDAC_SWITCH_REG3_REG = Display2_LCDDAC_VOLTAGE_SEL;
    Display2_LCDDAC_SWITCH_REG4_REG = Display2_LCDDAC_VOLTAGE_SEL;

    /* Turn on the LCD DAC and set the bias type */
    Display2_LCDDAC_CONTROL_REG = (Display2_LCDDAC_CONTROL_REG & Display2_BIAS_TYPE_MASK)
        | Display2_BIAS_TYPE;

    /* Set the contrast level for LCD with value chosen in the GUI */
    (void) Display2_SetBias(Display2_BIAS_VOLTAGE);

    /* Set lower bit of HI Drive strength to LCD Driver control register */
    Display2_DRIVER_CONTROL_REG |= ((uint8) (((uint8)(((uint8) Display2_HIDRIVE_STRENGTH) << 
                                Display2_LCDDRV_MODE0_SHIFT)) & ((uint8) Display2_LCDDRV_MODE0_MASK)));

    /* Set two high bits of HI Drive strength */
    Display2_WriteControlReg((uint8) (Display2_HIDRIVE_STRENGTH & Display2_MODE_MASK),
                                     (uint8) Display2_CNTL_MODE_MASK);

    /* ISR initialization */
    (void) CyIntDisable(Display2_ISR_NUMBER);

    /* Set the ISR to point to the TD_DoneInt Interrupt. */
    (void) CyIntSetVector(Display2_ISR_NUMBER, &Display2_ISR);

    /* Set the priority. */
    CyIntSetPriority(Display2_ISR_NUMBER, Display2_ISR_PRIORITY);
     
    #if(CY_PSOC5LP)
    
        /* Wakeup ISR initialization */
        CyIntDisable(Display2_WAKEUP_ISR_NUMBER);

        /* Set the ISR to point to the Wakeup_isr Interrupt. */
        (void) CyIntSetVector(Display2_WAKEUP_ISR_NUMBER, &Display2_WAKEUP_ISR);

        /* Set the priority of Wakeup ISR. */
        CyIntSetPriority(Display2_WAKEUP_ISR_NUMBER, Display2_WAKEUP_ISR_PRIORITY);
...
uint8 Display2_Enable(void) 
{
    ...
    Display2_PWR_MGR_REG |= Display2_LCD_EN;
    Display2_PWR_MGR_STBY_REG |= Display2_LCD_STBY_EN;

    CyExitCriticalSection(interruptState);

    /* Enable internal clock */
    Display2_Int_Clock_Enable();

    status = Display2_DmaConfigure();

    /* Set LCD PWM period */
    Display2_PWM_PERIOD_REG = Display2_PWM_PERIOD_VAL;
    Display2_PWM_DRIVE_REG = Display2_PWM_DRIVE_VAL;
    Display2_PWM_LODRIVE_REG = Display2_PWM_LOWDRIVE_VAL;

    Display2_WriteControlReg((uint8) Display2_CNTL_FRAME_DONE,
    (uint8)Display2_CNTL_FRAME_DONE_MASK);

    /* Global UDB enable, will be sent by DMA */
    Display2_WriteControlReg((uint8) Display2_CNTL_CLK_EN, (uint8) Display2_CNTL_CLK_EN_MASK);

    /* Global UDB enable, this required to start normal work as
    * it enables first DMA request.
    */
    Display2_CONTROL_REG |= Display2_CNTL_CLK_EN;

    #if(Display2_POWER_MODE != Display2__NO_SLEEP)

        /* Enable generation of lp_ack signal from UBD to PM */
        Display2_LCDDAC_CONTROL_REG |= Display2_LCDDAC_UDB_LP_EN;

        /* Start the LCD timer if in one of Low Power modes */
        Display2_TIMER_CONTROL_REG |= Display2_TIMER_EN;
...
/*******************************************************************************
* Function Name: Display2_DmaConfigure
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_lcdTd[] - used to store a set of allocated TD IDs that are
*  used by this component.
*
*  Display2_lcdChannel - stores a DMA Channel allocated for an LCD
*  usage.
*
*  Display2_frameTd[] - used to store a set of allocated TD IDs that
*  are used by this component for Frame signal generation.
*
*  Display2_LcdPort_DMA_TD_PROTO_BLOCK[] - used as a parameter for
*  DMA configuration.
*
* Reentrant:
*  No.
...
static uint8 Display2_DmaConfigure(void) CYREENTRANT
{
    ...
    uint32  dstAddr;
    /* DMA_PROTO_TD_DEF */
    static const struct Display2_DMA_TD_PROTO_ENTRY CYCODE Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[1u] =
    {
        {0u, 1u},
    };
    ...
    #if(CY_PSOC5)
        srcAddr = (uint32) Display2_frameTx;
        dstAddr = (uint32) Display2_CONTROL_PTR;
        
        /* Get the DMA channel for the Frame and check it for validness */
        Display2_frameChannel = Display2_Frame_Dma_DmaInitialize(0u, 0u, HI16(srcAddr), 
                                                                            HI16(Display2_CONTROL_REG_BASE));
    #else
        srcAddr = Display2_DMA_ADDRESS_MASK & ((uint32) Display2_frameTx);
        dstAddr = Display2_DMA_ADDRESS_MASK & ((uint32) Display2_CONTROL_PTR);
        
        /* Get the DMA channel for the Frame and check it for validness.
        * Upper 2 bytes of destination and source addrresses are always 0.
        */
        Display2_frameChannel = Display2_Frame_Dma_DmaInitialize(0u, 0u, 0u, 0u);
    #endif /* CY_PSOC5 */

    if(Display2_frameChannel == DMA_INVALID_CHANNEL)
    {
        status = CYRET_LOCKED;
    }
    ...
    */
        ...
        for(i = 0u; i < Display2_CONTROL_WRITE_COUNT; i++)
        {   /* Allocate a TD and check it if it valid. */
            Display2_frameTd[i]= CyDmaTdAllocate();
            
            if(DMA_INVALID_TD == Display2_frameTd[i])
            {
                status = CYRET_LOCKED;
            }
        }
        ...
        (void) CyDmaTdSetConfiguration(Display2_frameTd[0u],
                                       1u,
                                       Display2_frameTd[1u],
                                       TD_INC_SRC_ADR | Display2_Frame_Dma__TD_TERMOUT_EN);

        (void) CyDmaTdSetAddress(Display2_frameTd[0u], LO16(srcAddr), LO16(dstAddr));

        /* Set configuration for the second TD */
        (void) CyDmaTdSetConfiguration(Display2_frameTd[1u],
                                       1u,
                                       Display2_frameTd[0u],
                                       TD_INC_SRC_ADR | Display2_Frame_Dma__TD_TERMOUT_EN);

        (void) CyDmaTdSetAddress(Display2_frameTd[1u], LO16(srcAddr + 1u), LO16(dstAddr));
        ...
        for(i = 0u; i < Display2_CONTROL_WRITE_COUNT; i++)
        {
            if((Display2_CONTROL_WRITE_COUNT - 1u) != i)
            {
                (void) CyDmaTdSetConfiguration(Display2_frameTd[i],
                                               1u,
                                               Display2_frameTd[i + 1u],
                                               TD_INC_SRC_ADR | 
                                               Display2_Frame_Dma__TD_TERMOUT_EN);
            }
            else
            {
                (void) CyDmaTdSetConfiguration(Display2_frameTd[i],
            ...
            (void) CyDmaTdSetAddress(Display2_frameTd[i], LO16(srcAddr) + i , LO16(dstAddr));
        }
    ...
    {
        (void) CyDmaChSetInitialTd(Display2_frameChannel, Display2_frameTd[0u]);
        (void) CyDmaChEnable(Display2_frameChannel, 1u);
    }

    /* Conditional compilation is needed to avoid MISRA warning */
    #if(CY_PSOC5LP)
        srcAddr = (uint32) Display2_buffer;
        dstAddr = (uint32) Display2_ALIASED_AREA_PTR;
        
        Display2_lcdChannel = Display2_Lcd_Dma_DmaInitialize(0u, 0u, HI16(srcAddr), 
                                                                        HI16(Display2_ALIASED_AREA_PTR));
    #else
        srcAddr = Display2_DMA_ADDRESS_MASK & ((uint32) Display2_buffer);
        dstAddr = Display2_DMA_ADDRESS_MASK & ((uint32) Display2_ALIASED_AREA_PTR);
        
        Display2_lcdChannel = Display2_Lcd_Dma_DmaInitialize(0u, 0u, 0u, 0u);
    #endif /* CY_PSOC5 */

    if(Display2_lcdChannel == DMA_INVALID_CHANNEL)
    {
        status = CYRET_LOCKED;
    }

    for(j = 0u; j < Display2_COMM_NUM; j++)
    {
        #if(1 == Display2_SegLcdPort_DMA_TD_PROTO_COUNT)
                
            Display2_lcdTd[j] = CyDmaTdAllocate();
            
            if(DMA_INVALID_TD == Display2_lcdTd[j])
            {
                status = CYRET_LOCKED;
            }
            ...
            do{
                Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i] = CyDmaTdAllocate();
                
                if(DMA_INVALID_TD == Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i])
                {
                    status = CYRET_LOCKED;
                }
            ...
            } while(i < Display2_LCD_TD_SIZE);
            
        #endif /* (1 == Display2_SegLcdPort_DMA_TD_PROTO_COUNT) */
    }

    for(j = 0u; j < Display2_COMM_NUM; j++)
    {
        /* This is to supress MISRA warning. Depending on
        * Display2_SegLcdPort_DMA_TD_PROTO_COUNT need to differentiate
        * 2 or 3 configurations of LCD DMA transaction descriptors.
        */
        #if(1 == Display2_SegLcdPort_DMA_TD_PROTO_COUNT)
            
            if(Display2_COM_TD_COUNT == j)
            {
                /* The last TD need to point to TD[0] */
                (void) CyDmaTdSetConfiguration(Display2_lcdTd[j],
                                               (uint16)Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[0].length,
                                               Display2_lcdTd[0u],
                                               TD_INC_DST_ADR | TD_INC_SRC_ADR | 
                                               Display2_Lcd_Dma__TD_TERMOUT_EN);
            }
            ...
            */
            ...
            (void) CyDmaTdSetAddress(Display2_lcdTd[j],
                                     LO16(srcAddr) + (((uint16) j) * Display2_ROW_LENGTH) +
                                     ((uint16) Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[0].offset),
                                     LO16(dstAddr) + Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[0].offset);
        #else

            for(i = 0u; i < Display2_LCD_TD_SIZE; i++)
            {
                if(Display2_COM_TD_COUNT == ((j * Display2_LCD_TD_SIZE) + i))
                {
                    /* The last TD need to point to TD[0] */
                    (void) CyDmaTdSetConfiguration(Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i],
                                                   (uint16)Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[i].length,
                                                   Display2_lcdTd[0u],
                                                   TD_INC_DST_ADR | TD_INC_SRC_ADR | 
                                                   Display2_Lcd_Dma__TD_TERMOUT_EN);
                }
                ...
                * Each of this TDs should be seperatelly requested.
                */
                else if(i == (Display2_LCD_TD_SIZE - 1u))
                {
                    (void) CyDmaTdSetConfiguration(Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i],
                                                   (uint16) Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[i].length,
                                                   Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i + 1u],
                                                   TD_INC_DST_ADR | TD_INC_SRC_ADR | 
                                                   Display2_Lcd_Dma__TD_TERMOUT_EN);
                }
                /* Create a chain of TDs in which current TD ponts to next TD. 
                ...
                */
                ...
                (void) CyDmaTdSetAddress(Display2_lcdTd[(j * Display2_LCD_TD_SIZE) + i],
                                     LO16(srcAddr) + (((uint16) j) * Display2_ROW_LENGTH) +
                                     ((uint16) Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[i].offset),
                                      LO16(dstAddr) + Display2_SegLcdPort_DMA_TD_PROTO_BLOCK[i].offset);
            }
    ...
    if(CYRET_LOCKED != status)
    {
        (void) CyDmaChSetInitialTd(Display2_lcdChannel, Display2_lcdTd[0u]);
        (void) CyDmaChEnable(Display2_lcdChannel, 1u);
    }
...
/*******************************************************************************
* Function Name: Display2_DmaDispose
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_lcdTd[] - used to hold a set of allocated TD IDs that are
*  used by this component which will be released by this function.
*
*  Display2_lcdChannel- holds the channel number used by the LCD
*  component that will be released by this function.
*
*  Display2_frameTd[] - used to hold a set of allocated TD IDs that
...
static void Display2_DmaDispose(void) CYREENTRANT
{
    ...
    Display2_Lcd_Dma_DmaRelease();

    /* Free the LCD DMA channel */
    (void) CyDmaChFree(Display2_lcdChannel);

    /* Release LCD Frame DMA handle */
    Display2_Frame_Dma_DmaRelease();

    /* Free the channel used in generation Frame signal */
    (void) CyDmaChFree(Display2_frameChannel);

    #if(Display2_WAVEFORM_TYPE == Display2__TYPE_A)
        for(i = 0u; i < 2u; i++)
        {
            CyDmaTdFree(Display2_frameTd[i]);
        }
    #else
        for(i = 0u; i < (Display2_COMM_NUM * 2u); i++)
        {
            CyDmaTdFree(Display2_frameTd[i]);
        }
    #endif /* Display2_WAVEFORM_TYPE == Display2__TYPE_A */
    ...
    for(i = 0u; i < (Display2_COMM_NUM * Display2_LCD_TD_SIZE); i++)
    {
        CyDmaTdFree(Display2_lcdTd[i]);
    }
}
...
*
* Global variables:
*  Display2_initVar - is used to indicate initial configuration of
*  this component. The variable is initialized to zero and set to 1 the
*  first time Display2_Start() is called. This allows for component
*  initialization without re-initialization in all subsequent calls to the
*  Display2_Start() routine.
*
* Reentrant:
*  No.
*
...
void Display2_Stop(void) 
{
    ...
    /* Global UDB disable */
    Display2_WriteControlReg(0u, (uint8) Display2_CNTL_CLK_EN_MASK);

    #if(Display2_POWER_MODE != Display2__NO_SLEEP)

        /* Disable generation of lp_ack signal from UBD to PM */
        Display2_LCDDAC_CONTROL_REG &= ((uint8) ~Display2_LCDDAC_UDB_LP_EN);

        /* Disable LCD timer */
        Display2_TIMER_CONTROL_REG &= ((uint8) ~Display2_TIMER_EN);

    #endif /* Display2_POWER_MODE != Display2__NO_SLEEP */

    /* Clear Continous Drive */
    Display2_LCDDAC_CONTROL_REG &= ((uint8) ~Display2_LCDDAC_CONT_DRIVE);
    /* Clear Continous Display Blink bit */
    Display2_LCDDAC_CONTROL_REG &= ((uint8) ~Display2_LCDDRV_DISPLAY_BLNK);
    /* Clear Continous Bypass Enable bit */
    Display2_LCDDAC_CONTROL_REG &= ((uint8) ~Display2_LCDDRV_BYPASS_EN);

    interruptState = CyEnterCriticalSection();

    /* Disable LCD fixed function HW in PM */
    Display2_PWR_MGR_REG &= ((uint8) ~Display2_LCD_EN);
    Display2_PWR_MGR_STBY_REG &= ((uint8) ~Display2_LCD_STBY_EN);
    ...
    Display2_Int_Clock_Disable();
}


/*******************************************************************************
* Function Name: Display2_EnableInt
********************************************************************************
*
* Summary:
...
void Display2_EnableInt(void) 
{
    ...
    CyIntEnable(Display2_ISR_NUMBER);
    
    #if(CY_PSOC5LP)
    
        /* Enable Wakeup ISR interrupt */
        CyIntEnable(Display2_WAKEUP_ISR_NUMBER);
...
/*******************************************************************************
* Function Name: Display2_DisableInt
********************************************************************************
*
* Summary:
...
void Display2_DisableInt(void) 
{
    ...
    CyIntDisable(Display2_ISR_NUMBER);
...
/*******************************************************************************
* Function Name: Display2_SetBias
********************************************************************************
*
* Summary:
...
uint8 Display2_SetBias(uint8 biasLevel) 
{
    ...
    if(biasLevel < 64u)
    {
        Display2_CONTRAST_CONTROL_REG = biasLevel;
...
/*******************************************************************************
* Function Name: Display2_WriteInvertState
********************************************************************************
*
* Summary:
...
*
* Parameters:
*  invertState: the values can be - 0 (#Display2_NORMAL_STATE) for
*               normal noninverted display and 1
*                (#Display2_INVERTED_STATE) for inverted display.
*
* Return:
*  Status one of standard status for PSoC3 Component:
*      CYRET_SUCCESS - function completed successfully.
...
uint8 Display2_WriteInvertState(uint8 invertState) 
{
    ...
    */
    if(invertState <= Display2_INVERTED_STATE)
    {
        if(invertState == Display2_NORMAL_STATE)
        {
            pixelState = Display2_PIXEL_STATE_OFF;
        }
        else
        {
            pixelState = Display2_PIXEL_STATE_ON;
        }

        #if defined(Display2_GANG)

            for(i = 0u; i < Display2_COMM_NUM; i++)
            {
                pixelNumber = Display2_gCommons[i];

                for(j = 0u; j < Display2_COMM_NUM; j++)
                {
                    pixelNumber = (pixelNumber & ((uint16) ~Display2_ROW_MASK)) | 
                                  (((uint16) j) << Display2_ROW_SHIFT);
                    
                    Display2_WRITE_PIXEL(pixelNumber, pixelState);
                }
            }

            for(i = 0u; i < Display2_COMM_NUM; i++)
            {
                Display2_WRITE_PIXEL(Display2_gCommons[i],
                                            (((uint8) ~pixelState) & ((uint8)Display2_STATE_MASK)));
            }
        ...
        for(i = 0u; i < Display2_COMM_NUM; i++)
        {
            pixelNumber = Display2_commons[i];
        ...
        for(i = 0u; i < Display2_COMM_NUM; i++)
        {
            Display2_WRITE_PIXEL(Display2_commons[i],
                                        (((uint8)~pixelState) & ((uint8)Display2_STATE_MASK)));
        }

        Display2_DRIVER_CONTROL_REG = (Display2_DRIVER_CONTROL_REG & 
                                              ((uint8)Display2_STATE_MASK)) | 
                                              ((uint8)(invertState << Display2_INVERT_SHIFT));
...
/*******************************************************************************
* Function Name: Display2_ReadInvertState
********************************************************************************
*
* Summary:
...
*
* Return:
*  State of the LCD - 0(#Display2_NORMAL_STATE) for normal non-inverted
*  display and 1(#Display2_INVERTED_STATE) for inverted display.
*
*******************************************************************************/
uint8 Display2_ReadInvertState(void) 
{
    /* Get only invert bit of Driver Control Register */
    return((Display2_DRIVER_CONTROL_REG & Display2_INVERT_BIT_MASK) >> Display2_INVERT_SHIFT);
}


/*******************************************************************************
* Function Name: Display2_ClearDisplay
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_buffer[] - an array is modified by this API. Clears the
*  frame buffer to all zeroes and then reinitialize it with a 1 in the
*  locations specified by the values from Display2_commons[].
*
*  Display2_commons[] - holds the pixel locations for common lines.
*
* Reentrant:
*  No.
*
...
void Display2_ClearDisplay(void) 
{
    ...
    for(i = 0u; i < Display2_BUFFER_LENGTH; i++)
    {
        Display2_buffer[i] = 0u;
    }
    ...
    {
        #ifdef Display2_GANG
            Display2_WRITE_PIXEL(Display2_gCommons[i], Display2_PIXEL_STATE_ON);
        #endif /* Display2_GANG */

        Display2_WRITE_PIXEL(Display2_commons[i], Display2_PIXEL_STATE_ON);
    }

    /* Read invert state. */
    dispState = (uint8)(((uint8)(Display2_DRIVER_CONTROL_REG &((uint8) ~Display2_STATE_MASK))) >> 
                Display2_INVERT_SHIFT);
    ...
    * display.
    */
    if(dispState == Display2_INVERTED_STATE)
    {
        (void) Display2_WriteInvertState(dispState);
    }
}


/*******************************************************************************
* Function Name: Display2_WritePixel
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_buffer[] - an array is modified by this API. This API
...
uint8 Display2_WritePixel(uint16 pixelNumber, uint8 pixelState) 
{
    ...
    if((Display2_NOT_CON != pixelNumber) || (!(pixelState > Display2_PIXEL_STATE_INVERT)))
    {
        if(pixelState == Display2_PIXEL_STATE_INVERT)
        {
            /* Invert actual pixel state */
            pixelState = Display2_ReadPixel(pixelNumber);
        ...
        * buffer.
        */
        row = Display2_EXTRACT_ROW(pixelNumber);
        port = Display2_EXTRACT_PORT(pixelNumber);
        pin = Display2_EXTRACT_PIN(pixelNumber);

        /* Write new pixel's value to the frame buffer. */
        mask = (uint8) ~((uint8)(((uint8)Display2_PIXEL_STATE_ON) << pin));

        /* Write new pixel's value to the frame buffer. */
        Display2_buffer[(row * Display2_MAX_BUFF_ROWS) + port] =
            (Display2_buffer[(row * Display2_MAX_BUFF_ROWS) + port] & mask) | 
...
/*******************************************************************************
* Function Name: Display2_ReadPixel
********************************************************************************
*
* Summary:
...
* Return:
*  Pixel State:
*       Display2_PIXEL_STATE_OFF - for pixel off.
*       Display2_PIXEL_STATE_ON - for pixel on.
*       Display2_PIXEL_UNKNOWN_STATE - for not assigned pixel.
*
* Global variables:
*  Display2_buffer[] - holds the state of every pixel of the LCD glass
...
uint8 Display2_ReadPixel(uint16 pixelNumber) 
{
    ...
    if(pixelNumber != Display2_NOT_CON)
    {
        /* Extract the pixel information to locate desired pixel in the frame
        * buffer.
        */
        ...
        pixelState = (((Display2_buffer[(row * Display2_MAX_BUFF_ROWS) + port]) >> pin) &
                    Display2_PIXEL_STATE_ON);
    }
    else
    {
        pixelState = Display2_PIXEL_UNKNOWN_STATE;
    }
...
/*******************************************************************************
* Function Name: Display2_Write7SegDigit_0
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_7SegDigits[] - used as a look-up table for 7 Segment helper.
*  Holds decoded digit's pixel reflection for the helper.
*
*  Display2_disp0[][] - holds pixels locations for 7 Segment helper in 
*  the Frame Buffer.
*
*  Display2_DIGIT_NUM_0 - holds the maximum digit number for the helper.
*
* Reentrant:
*  No.
*
...
{
    static const uint16 CYCODE Display2_disp0[1u][7u] = {{Display2_H7SEG0_A, Display2_H7SEG0_B, Display2_H7SEG0_C,
    Display2_H7SEG0_D, Display2_H7SEG0_E, Display2_H7SEG0_F, Display2_H7SEG0_G}};
    uint8 i;
    ...
    if((position / Display2_DIGIT_NUM_0) == 0u)
    {
        position = Display2_DIGIT_NUM_0 - position - 1u;
        for(i = 0u; i < Display2_7SEG_PIX_NUM; i++)
        {
            /* Select pixel state for the digit base on value in look-up table */
            Display2_WRITE_PIXEL(Display2_disp0[position][i],
            ((uint8)(Display2_7SegDigits[digit] >> i)) & ((uint8)Display2_PIXEL_STATE_ON));
        }
    }
}
...
/*******************************************************************************
* Function Name: Display2_Write7SegNumber_0
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Display2_WriteControlReg
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Display2_frameTx[] - used to store a set of precalculated values
...
static void Display2_WriteControlReg(uint8 value, uint8 mask) CYREENTRANT
{
    ...
    for(i = 0u; i < Display2_CONTROL_WRITE_COUNT; i++)
    {
        ...
        Display2_frameTx[i] = (Display2_frameTx[i] & mask) | value;
    }
}
