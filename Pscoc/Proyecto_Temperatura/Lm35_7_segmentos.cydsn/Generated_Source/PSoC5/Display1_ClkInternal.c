/*******************************************************************************
* File Name: Display1_ClkInternal.c
* Version 2.20
*
*  Description:
...
#include "Display1_ClkInternal.h"
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_Start(void) 
{
    /* Set the bit to enable the clock. */
    Display1_ClkInternal_CLKEN |= Display1_ClkInternal_CLKEN_MASK;
	Display1_ClkInternal_CLKSTBY |= Display1_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Display1_ClkInternal_CLKEN &= (uint8)(~Display1_ClkInternal_CLKEN_MASK);
	Display1_ClkInternal_CLKSTBY &= (uint8)(~Display1_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_StopBlock(void) 
{
    if ((Display1_ClkInternal_CLKEN & Display1_ClkInternal_CLKEN_MASK) != 0u)
    {
...
#if defined(Display1_ClkInternal__CFG3)
        CLK_DIST_AMASK = Display1_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Display1_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Display1_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Display1_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Display1_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Display1_ClkInternal_CLKSTBY &= (uint8)(~Display1_ClkInternal_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Display1_ClkInternal_GetSourceRegister();
    uint16 oldDivider = Display1_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Display1_ClkInternal_CLKEN & Display1_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Display1_ClkInternal_DIV_PTR, clkDivider);
                Display1_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Display1_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Display1_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Display1_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Display1_ClkInternal_DIV_PTR, clkDivider);
				Display1_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Display1_ClkInternal_GetDividerRegister(void) 
{
    return CY_GET_REG16(Display1_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_SetModeRegister(uint8 modeBitMask) 
{
    Display1_ClkInternal_MOD_SRC |= modeBitMask & (uint8)Display1_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_ClearModeRegister(uint8 modeBitMask) 
{
    Display1_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Display1_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display1_ClkInternal_GetModeRegister(void) 
{
    return Display1_ClkInternal_MOD_SRC & (uint8)(Display1_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Display1_ClkInternal_GetDividerRegister();
    uint8 oldSrc = Display1_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Display1_ClkInternal_MOD_SRC |= CYCLK_SSS;
        Display1_ClkInternal_MOD_SRC =
            (Display1_ClkInternal_MOD_SRC & (uint8)(~Display1_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display1_ClkInternal_GetSourceRegister(void) 
{
    return Display1_ClkInternal_MOD_SRC & Display1_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Display1_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Display1_ClkInternal_SetPhaseRegister(uint8 clkPhase) 
{
    Display1_ClkInternal_PHASE = clkPhase & Display1_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Display1_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Display1_ClkInternal_GetPhaseRegister(void) 
{
    return Display1_ClkInternal_PHASE & Display1_ClkInternal_PHASE_MASK;
}
