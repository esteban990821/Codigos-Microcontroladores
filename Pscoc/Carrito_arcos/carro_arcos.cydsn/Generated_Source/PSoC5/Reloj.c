...
void Reloj_Start(void) 
{
    ...
    Reloj_CLKEN |= Reloj_CLKEN_MASK;
	Reloj_CLKSTBY |= Reloj_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Reloj_Stop
********************************************************************************
*
* Summary:
...
void Reloj_Stop(void) 
{
    ...
    Reloj_CLKEN &= (uint8)(~Reloj_CLKEN_MASK);
	Reloj_CLKSTBY &= (uint8)(~Reloj_CLKSTBY_MASK);
}
...
{
    if ((Reloj_CLKEN & Reloj_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Reloj_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Reloj_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Reloj_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Reloj_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Reloj_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Reloj_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Reloj_GetSourceRegister();
    uint16 oldDivider = Reloj_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Reloj_CLKEN & Reloj_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Reloj_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Reloj_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Reloj_DIV_PTR);
}


/*******************************************************************************
* Function Name: Reloj_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_MOD_SRC |= modeBitMask & (uint8)Reloj_MODE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Reloj_MODE_MASK));
}


/*******************************************************************************
* Function Name: Reloj_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_MOD_SRC & (uint8)(Reloj_MODE_MASK);
}


/*******************************************************************************
* Function Name: Reloj_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Reloj_GetDividerRegister();
    uint8 oldSrc = Reloj_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Reloj_MOD_SRC |= CYCLK_SSS;
        Reloj_MOD_SRC =
            (Reloj_MOD_SRC & (uint8)(~Reloj_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Reloj_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_MOD_SRC & Reloj_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Reloj_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_PHASE = clkPhase & Reloj_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Reloj_PHASE & Reloj_PHASE_MASK;
}
